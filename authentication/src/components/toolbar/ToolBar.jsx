...
const ToolBar = () => {
  ...
  async function deleteUser() {
    ...
    for (let i = 0; i < rows.length; i++) {
      let inputChecked =
        rows[i].cells[0].getElementsByTagName("input")[0].checked;
      if (inputChecked) {
        const getName = rows[i].cells[1].innerHTML.split("<br>");
        usersNames.push(getName[0]);
      }
    }

    usersNames.forEach((name) => {
      fetch(`http://localhost:4200/users/${name}`, { method: "DELETE" });
    });
